...
def generate_launch_description():
    ...
    gpsd_script_path = os.path.join(
        get_package_share_directory("navsatfix2nmea"), "scripts", "gpsd_tcp.sh"
    )
    ...
    gpsd = ExecuteProcess(
        cmd=[gpsd_script_path, gps_ip, local_gps_port], output="screen"
    )
    ...
    restart_on_gpsd_exit = RegisterEventHandler(
        OnProcessExit(
            target_action=gpsd,
            on_exit=[EmitEvent(event=Shutdown(reason="gpsd died, restarting launch"))],
        )
    )

    return LaunchDescription(
        [gps_ip_arg, local_gps_port_arg, navsatfix2nmea, gpsd, restart_on_gpsd_exit]
    )
